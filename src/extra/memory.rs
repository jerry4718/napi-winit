    ...
    impl DynamicBuffer {
        pub fn new(count: f64, per: f64, ratio: Option<f64>) -> Result<Self> {
            if count != count.floor() || count <= 0. {
                return Err(Error::from_reason("The count is not an int or lower than 0"));
            }
            if per != per.floor() || per <= 0. {
                return Err(Error::from_reason("The per is not an int or lower than 0"));
            }
            let ratio = ratio.unwrap_or(1.5);
            
            if ratio <= 1. {
                return Err(Error::from_reason("The ratio must be greater than 1"));
            }

            let init_size = count * per * ratio;

            if init_size % per != 0. {
                return Err(Error::from_reason("The init_size is not a multiple of per"));
            }
