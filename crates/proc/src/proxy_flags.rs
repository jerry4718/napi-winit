use crate::utils::{
    append_to_tokens, get_meta_by_name, get_meta_value_as, get_type_ty_or, parse_as, parse_metas,
};
use proc_macro2::TokenStream;
use quote::{ToTokens, format_ident, quote, quote_spanned};
use syn::{ExprTuple, Ident, ItemStruct, Type, parse_macro_input};

pub(crate) fn proxy_flags(
    ...
    let Some(flags_meta) = bitflags else {
        panic!("`flags` must assigned on #[proxy_flags]")
    };

    let Some(expr_tuple) = get_meta_value_as::<ExprTuple>(&flags_meta) else {
        panic!("value for `flags` must like a tuple (FLAG_A, FLAG_B, FLAG_C)")
    };

    let flags: Vec<_> = expr_tuple
        .elems
        .iter()
        .map(|el| parse_as::<Ident>(el))
        .collect();
...
impl ToTokens for ProxyFlags {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        ...
        let lower_names: Vec<_> = flags
            .iter()
            .map(|flag| format_ident!("{}", flag.to_string().to_lowercase()))
            .collect();
        ...
        let zip2: Vec<_> = flags
            .iter()
            .zip(lower_names.iter().zip(&flag_idents))
            .collect();
        ...
        let flag_dispose: Vec<_> = zip2
            .iter()
            .map(|(flag, (_, field_ident))| {
                quote_spanned!( flag.span() =>
