...
pub(crate) fn get_metas_by_attr_name(attrs: &[Attribute], name: &str) -> Vec<Meta> {
    attrs
        .iter()
        .filter(|attr| matches!(attr.path().get_ident(), Some(ident) if ident == name))
        .flat_map(|attr| attr.parse_args_with(parse_to_metas).unwrap_or(vec![]))
        .collect()
}

pub(crate) fn get_meta_by_name(metas: &[Meta], name: &str) -> Option<Meta> {
    metas
        .iter()
...
pub(crate) fn get_ident_optional(meta: &Option<Meta>) -> Option<Ident> {
    ...
    let Some(ident) = get_meta_value_as(meta) else {
