use crate::{
    conf_usage::{ConfUsage, get_meta_value_as_conf_usage},
    utils::{
        append_to_tokens, define_const_str, get_meta_value_as, get_metas_by_attr_name,
        map_meta_to_local, separate_attr_by_name,
    },
};
...
pub(crate) fn parse_conf_fields(
    fields: &Fields,
...
pub(crate) fn quote_conf_fields<F>(
    ConfFields { fields, kind }: &ConfFields,
    pose: Kind,
    with: With,
    mapper: F,
) -> TokenStream
    ...
    F: Fn(&ConfField) -> TokenStream,
{
    ...
    match (with, pose, kind) {
        ...
        (With::Origin, Kind::Compose, FieldsKind::Named) => {
            quote!({ #( #field_idents: #field_tokens ),* })
        }
...
define_const_str!(META_NAMED = Named, META_UNNAMED = Unnamed,);

pub(crate) fn parse_fields_kind(
    fields: &Fields,
    attrs: &[Attribute],
    attr_proxy_name: &str,
) -> FieldsKind {
